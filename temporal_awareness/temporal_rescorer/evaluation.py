...
def eval_pipeline(run_llm_judge: bool, file_path, log_to_wandb, save_path, specific_k=None):
    ...
    for df, label in zip(dfs_to_return, df_labels):
        ...
        save_to_path = save_path + "/evaluation_results"
        os.makedirs(save_to_path, exist_ok=True)

        summary_df.to_csv(f"{save_to_path}/{model_name}_{label}_aggregate_metrics.csv")
        df.to_csv(f"{save_to_path}/{model_name}_{label}_detailed_metrics.csv")
...
def main(use_llm_judge, file_path, log_to_wandb, save_path):
    eval_pipeline(run_llm_judge = use_llm_judge, file_path=file_path, log_to_wandb=log_to_wandb, save_path=save_path)
...
if __name__ == "__main__":
     ...
     main(use_llm_judge=args.llm, file_path=args.f, log_to_wandb=args.log_to_wandb, save_path=args.save_path)
